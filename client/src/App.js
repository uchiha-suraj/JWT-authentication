...
function App() {
  ...
  // checks if refresh token is valid or not, creates an access token and returns it.
  const refresh = refreshToken => {
    console.log("Refreshing token");

    return new Promise((resolve, reject) => {
      axios.post("http://localhost:3000/refresh", { token: refreshToken }).then(data => {
  ...
  // checks if cookies are valid or not, based on this we would be changing access token.
